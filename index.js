...
try {
    ...
    console.error("Erro ao conectar DB:", e.message);
}
...
async function analisarFaturaGemini(mediaUrl, mimeType) {
    try {
        console.log(`\n[IA GEMINI] 📥 Iniciando leitura do documento...`);
...
// 🔥 VARREDURA COM FUSÃO INTELIGENTE
async function varreduraIgreenDiaria() {
    ...
    try {
        ...
        try { 
            await pageIgreen.evaluate(() => { const btn = Array.from(document.querySelectorAll('button, div')).find(el => el.textContent.includes('Agora não')); if(btn) btn.click(); }); 
            await new Promise(r => setTimeout(r, 2000)); 
        } catch(e){}
...
async function fluxoProcessamentoUniversal(mediaUrl, mimeType, phone, cpfAlvo = null) {
    ...
    try {
        ...
        try { 
            ...
            await enviarMensagem(phone, "⚠️ A Inteligência Artificial teve dificuldade em ler o arquivo."); 
            return; 
        }
        ...
        if (!cpfFinal) { 
            await enviarMensagem(phone, "⚠️ CPF não identificado na fatura. Salvo no banco, mas injeção na iGreen abortada."); 
            return; 
        }
    ...
    try {
        ...
        if (!cpf || !nascimento) {
            ...
            let extraidosEsq = await pageIgreen.evaluate((busca) => { 
                ...
                if(!l) return { texto: "", codigo: "", nome: "" }; 
                const cols = Array.from(l.querySelectorAll('.MuiDataGrid-cell, td')); 
                return { texto: l.textContent, codigo: cols[0]?.textContent?.trim() || "", nome: cols[1]?.textContent?.trim() || "" }; 
            }, termoBuscaIgreen);
        ...
        for (let tentativa = 1; tentativa <= 3; tentativa++) {
            ...
            try {
                ...
                if (ucIdentificada) { await salvarNoBanco(cpf, phone, { UC_ATUALIZADA_EQUATORIAL: ucIdentificada, UC: ucIdentificada }); } 
                await new Promise(r => setTimeout(r, 5000));
        ...
        for (let clique = 0; clique < 3; clique++) { 
            await pageIgreenFinal.evaluate(() => { const botoesAcao = Array.from(document.querySelectorAll('button, span, a, div')).filter(el => el.textContent.trim() === 'Realizar ação' || el.textContent.includes('Realizar ação')); const btn = botoesAcao.filter(b => b.offsetParent !== null).pop() || botoesAcao[botoesAcao.length - 1]; if(btn) { btn.scrollIntoView({behavior: 'smooth', block: 'center'}); btn.click(); } }); 
            await new Promise(r => setTimeout(r, 3000)); 
        }
        ...
        if (inputUploads.length > 0) { 
            for (let input of inputUploads) { 
                try { await input.uploadFile(caminhoFaturaLocal); await pageIgreenFinal.evaluate((el) => el.dispatchEvent(new Event('change', { bubbles: true })), input); } catch(e){} 
            } 
        } else { 
            throw new Error("O formulário de anexo da iGreen está bloqueado ou invisível."); 
        }
...
function iniciarMotorRecorrente() {
    ...
    // Inicia a varredura primária 15 segundos após ligar
    setTimeout(() => { varreduraIgreenDiaria(); }, 15000);
}

iniciarMotorRecorrente();

// 🔥 ROTAS WEBHOOK E EXPRESS
app.post('/webhook/igreen', async (req, res) => {
    ...
    switch (mem.STATUS_CADASTRO) {
        ...
        case 'AGUARDANDO_FATURA_DEVOLUTIVA_MANUAL': { 
            if (temMidia) { const cpfAlvo = mem.cpfAlvo; memoriaEstado.delete(phone); fluxoProcessamentoUniversal(mediaUrl, mimeType, phone, cpfAlvo); } else { await enviarMensagem(phone, "⚠️ Envie a fatura."); } 
            break; 
        }
        case 'AGUARDANDO_FATURA_SOH_BANCO': { 
            if (temMidia) { 
                ...
                try { 
                    ...
                    await enviarMensagem(phone, "❌ Erro na análise."); 
                } 
            } else { await enviarMensagem(phone, "⚠️ Aguardando fatura."); } 
            break; 
        }
        ...
        case 'AGUARDANDO_EMAIL': { 
            ...
            } else { await enviarMensagem(phone, "⚠️ Digite um e-mail."); } 
            break; 
        }
        case 'AGUARDANDO_DADOS_DEVOLUTIVA': { 
            ...
            } else { await enviarMensagem(phone, "⚠️ Digite o Nome/ID."); } 
            break; 
        }
        case 'AGUARDANDO_UC_DOC': { 
            ...
            } else { await enviarMensagem(phone, "⚠️ Digite a UC."); } 
            break; 
        }
        case 'OP4_PEDIR_TELEFONE': { 
            ...
            } else { await enviarMensagem(phone, "⚠️ Telefone inválido."); } 
            break; 
        }
        case 'OP4_PEDIR_EMAIL': { 
            if (textoIn.includes('@')) { 
                ...
                memoriaEstado.set(phone, { STATUS_CADASTRO: 'AGUARDANDO_DOC_FRENTE', docId: mem.docId }); 
                await enviarMensagem(phone, TEXTOS.T_PEDIR_FOTO_DOC_FRENTE); 
            } else { await enviarMensagem(phone, "⚠️ E-mail inválido."); } 
            break; 
        }
        case 'AGUARDANDO_DOC_FRENTE': { 
            ...
            } else { await enviarMensagem(phone, "⚠️ Envie a FRENTE."); } 
            break; 
        }
        case 'AGUARDANDO_DOC_VERSO': { 
            if (temMidia) { 
                ...
                memoriaEstado.delete(phone); 
            } else { await enviarMensagem(phone, "⚠️ Envie o VERSO."); } 
            break; 
        }
    }
});
...
app.get('/tela-robo', (req, res) => { 
    ...
    if (fs.existsSync(file)) { res.contentType('image/png'); res.sendFile(path.resolve(file)); } else { res.status(404).send('Sem imagem'); } 
});

app.get('/ultima-fatura', (req, res) => { 
    const file = path.join('/tmp', 'ultima_fatura.pdf'); 
    if (fs.existsSync(file)) { res.contentType('application/pdf'); res.sendFile(path.resolve(file)); } else { res.status(404).send('Sem fatura'); } 
});
...
async function validateBrowser() { 
    try { 
        ...
        console.error('❌ Browser falhou:', error.message);
        process.exit(1); 
    } 
}
