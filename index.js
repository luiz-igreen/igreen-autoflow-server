...
} catch (e) { console.error("[FIREBASE] Erro DB:", e.message); }
...
const CHROME_ARGS = ["--no-sandbox", "--disable-setuid-sandbox", "--disable-dev-shm-usage", "--disable-gpu", "--single-process", "--no-zygote", "--js-flags=--expose-gc"]; 
...
async function enviarMensagem(phone, message) {
  ...
  try { 
    ...
    console.error(`[Z-API] Erro ao enviar: ${e.message}`);
  }
}
...
async function salvarNoBanco(phone, dados) {
  if (admin.apps.length > 0) {
    ...
    } catch (e) { console.error("[FIREBASE] Erro ao salvar:", e.message); }
  }
}
...
async function analisarFaturaGemini(mediaUrl, mimeType) {
  if (!GEMINI_API_KEY) {
    console.error("[GEMINI] ERRO FATAL: chave ausente.");
  ...
  try {
    ...
    throw new Error("Não consegui baixar o documento.");
  }

  const prompt = `
    Aja como um auditor de faturas de energia elétrica da iGreen Energy.
    Extraia os seguintes dados da fatura fornecida e retorne APENAS um JSON válido, sem formatações Markdown.
    Chaves exigidas:
    - "NOME_CLIENTE"
    - "CPF"
    - "DISTRIBUIDORA"
  ...
  try {
    ...
    console.error(`[GEMINI] Erro (${status}): ${msg}`);
    throw new Error(`Erro ao processar documento: ${msg}`);
  }
}
...
async function fluxoExtracaoDados(termoBusca, phone) {
  ...
  try {
    console.log(`[EXTRATOR] Iniciando Navegador RPA...`);
...
app.listen(PORT, () => console.log(`[SERVIDOR] 🚀 IGREEN ONLINE (Gemini Pro v1)`));
